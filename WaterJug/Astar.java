...
class State {
    ...
    int g; // Path cost to reach this state
    int h; // Heuristic value

    public State(int x, int y, State parent, int g, int h) {
        this.x = x;
        this.y = y;
        this.parent = parent;
        this.g = g;
    ...
    public ArrayList<State> getNextStates(int maxX, int maxY, int targetX, int targetY) {
        ...
        // Fill jug x
        nextStates.add(new State(maxX, y, this, g + 1, getHeuristic(maxX, y, targetX, targetY)));
        // Fill jug y
        nextStates.add(new State(x, maxY, this, g + 1, getHeuristic(x, maxY, targetX, targetY)));
        // Empty jug x
        nextStates.add(new State(0, y, this, g + 1, getHeuristic(0, y, targetX, targetY)));
        // Empty jug y
        nextStates.add(new State(x, 0, this, g + 1, getHeuristic(x, 0, targetX, targetY)));
        // Pour from x to y
        int pourXY = Math.min(x, maxY - y);
        nextStates.add(new State(x - pourXY, y + pourXY, this, g + 1, getHeuristic(x - pourXY, y + pourXY, targetX, targetY)));
        // Pour from y to x
        int pourYX = Math.min(y, maxX - x);
        nextStates.add(new State(x + pourYX, y - pourYX, this, g + 1, getHeuristic(x + pourYX, y - pourYX, targetX, targetY)));
...
public class Astar {
    ...
    public static void main(String[] args) {
        ...
        State initialState = new State(0, 0, null, 0, Math.abs(targetX - 0) + Math.abs(targetY - 0)); // Initial state

        // A* Search Algorithm
        PriorityQueue<State> open = new PriorityQueue<>(Comparator.comparingInt(a->a.h+a.g));
